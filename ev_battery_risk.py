...
def run_montecarlo():
    ...
    for _ in range(n_warmup):
        soc_fine_op, _, _, _, _, _, _, _, _, _, _, _, _, _, _, _, _, _, _, _, _ = simula_giornata(soc_wup, registra=False)
    ...
    all_slot_extra3 = []
    all_slot_extra4 = []
    n_ric_arr        = np.zeros(n_sim)   # ricariche per giornata

    SLOT_24H = int(24 * 60 / SLOT_MIN)
    denom_saturazione = SLOT_24H * int(num_pdr)

    for s in range(n_sim):
        soc_fine, soc_min, en_m, n_stress, slot_occ, kwh_er, n_bp, flag_bp, s_batt_att, s_batt_sog, s_pu_bind, s_st_bind, s_dist, d_rich, d_kwmax, n_bp_b, flag_bp_b, d_b, se3, se4, n_ric_s = simula_giornata(soc_start, registra=True, idx=s)
