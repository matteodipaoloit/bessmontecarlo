...
with st.sidebar:
    ...
    kw_soglia_sistema = st.number_input(
        ...
        help="Episodio se (rete+batt)/n_attivi < soglia E kw_max_auto × coeff_picco > soglia_auto."
    )
    ...
    if distribuzione == "Doppia Gaussiana (mattina + sera)":
        ...
        with c1:
            ...
            sigma1 = st.slider("Ampiezza mat. (h)", 0.5, 8.0, 4.0, 0.5)
            peso1  = st.slider("% mattina", 10, 90, 60)
        with c2:
            picco2 = st.slider("Picco sera (h)", ORA_INIZIO, ORA_FINE - 1, p2_default)
            sigma2 = st.slider("Ampiezza sera (h)", 0.5, 8.0, 2.0, 0.5)
...
def run_montecarlo():
    ...
    pre_slot   = [rng_sim.choice(N_SLOT, size=ricariche_tot, p=weights)   for _ in range(n_sim)]
    pre_kwh    = [np.clip(
                    rng_sim.normal(kwh_medi_ricarica, kwh_medi_ricarica * sigma_pct / 100, size=ricariche_tot),
                    kwh_medi_ricarica * clip_min_pct / 100,
                    kwh_medi_ricarica * clip_max_pct / 100)
                  for _ in range(n_sim)]
    pre_kwmax  = [np.clip(
                    rng_sim.normal(kw_auto_media, kw_auto_sigma, size=ricariche_tot),
    ...
    def simula_giornata(soc_inizio, registra=False, idx=0):
        ...
        if registra:
            slot_arrivo     = pre_slot[idx]
            kwh_target      = pre_kwh[idx].copy()
            kw_max_per_auto = pre_kwmax[idx].copy()
        else:
        ...
        for t in range(N_SLOT):
            ...
            kw_per_stallo_disp = kw_totale_disp / n_attivi if n_attivi > 0 else kw_totale_disp
            for i_st, st_ in enumerate(stalli):
                ...
                if not attivo:
                    ...
                    if kwh_def3[i_st] > 0 or kwh_def4[i_st] > 0:
                        kw_r = st_['kw_max'] * coeff_picco
                        kw_n = st_['kw_max']
                        if kw_r > 0:
                            slot_extra3_list.append(kwh_def3[i_st] / kw_r / SLOT_H)
                        if kw_n > 0:
                    ...
                    if (kw_per_stallo_disp < kw_richiesto
                            and kw_richiesto > kw_soglia_auto
                            and kw_per_stallo_disp < kw_soglia_sistema
                            and not stallo_in_bassa[i_st]):
                        n_episodi_bassa += 1
                        giorno_ha_bassa  = True
                        stallo_in_bassa[i_st] = True
                        delta_richiesto_list.append(kw_richiesto - kw_per_stallo_disp)
                        delta_kwmax_list.append(st_['kw_max'] - kw_per_stallo_disp)

                    # Tipo B: kw_per_stallo_disp < kw_richiesto AND < P_stallo
                    if (kw_per_stallo_disp < kw_richiesto
                            and kw_per_stallo_disp < potenza_max_stallo
                            and not stallo_in_b[i_st]):
                        n_episodi_b += 1
                        giorno_ha_b  = True
                        stallo_in_b[i_st] = True
                        delta_b_list.append(kw_richiesto - kw_per_stallo_disp)

                    # Accumula kWh deficit slot per slot (Opz 3 e 4)
                    def3 = max(0.0, kw_richiesto - kw_per_stallo_disp) * SLOT_H
                    def4 = max(0.0, st_['kw_max'] - kw_per_stallo_disp) * SLOT_H
                    kwh_def3[i_st] += def3
                    kwh_def4[i_st] += def4
            ...
            for i_st, st_ in enumerate(stalli):
                if st_['kwh'] > 0.0:
                    ...
                    # Debug: quale vincolo ha limitato questo stallo?
                    domanda_raw = min(potenza_max_stallo, st_['kw_max'], st_['kwh'] / SLOT_H + kwh_er / SLOT_H)
                    quota_glob  = kw_totale_disp / n_attivi if n_attivi > 0 else 0.0
                    n_att_pu    = attivi_per_pu.get(st_['pu'], 1)
                    quota_pu    = potenza_pu_kw / n_att_pu
                    if kw_limit < domanda_raw - 1e-6:  # stallo non era soddisfatto
                        if quota_pu < quota_glob:
                            slot_pu_binding += 1      # PU era il vincolo più stringente
                        # else: globale era il vincolo
                    if kw_limit >= domanda_raw - 1e-6 and domanda_raw >= potenza_max_stallo - 1e-6:
                        slot_stallo_binding += 1      # limite stallo raggiunto
        ...
        return soc, soc_min, en_mancante, n_slot_stress, slot_occupati, kwh_erogati_tot, n_episodi_bassa, giorno_ha_bassa, slot_batt_attiva, slot_batt_soglia, slot_pu_binding, slot_stallo_binding, dist_attivi, delta_richiesto_list, delta_kwmax_list, n_episodi_b, giorno_ha_b, delta_b_list, slot_extra3_list, slot_extra4_list
    ...
    for _ in range(n_warmup):
        soc_fine_op, _, _, _, _, _, _, _, _, _, _, _, _, _, _, _, _, _, _, _ = simula_giornata(soc_wup, registra=False)
    ...
    for s in range(n_sim):
        soc_fine, soc_min, en_m, n_stress, slot_occ, kwh_er, n_bp, flag_bp, s_batt_att, s_batt_sog, s_pu_bind, s_st_bind, s_dist, d_rich, d_kwmax, n_bp_b, flag_bp_b, d_b, se3, se4 = simula_giornata(soc_start, registra=True, idx=s)
    ...
    return {
        ...
        "batt_attiva_pct":     float(np.mean(batt_attiva_arr)) / N_SLOT * 100,
        "batt_soglia_pct":     float(np.mean(batt_soglia_arr)) / N_SLOT * 100,
        # Debug PU e stallo
...
# ─── RISULTATI ───────────────────────────────────────────────────────────────
    ...
    with tab_a:
        st.caption(
            f"Condizione: **(rete+batt)/n_attivi < kW_max×coeff** "
            f"AND **kW_max×coeff > {kw_soglia_auto:.0f} kW** (S_auto) "
            f"AND **(rete+batt)/n_attivi < {kw_soglia_sistema:.0f} kW** (S_sis) "
        ...
        with bpa1:
            st.markdown(f'<div class="metric-card {bp_cls}"><b>Giornate critiche Tipo A</b><br>'
                        ...
                        f'(rete+batt)/stalli_attivi &lt; {kw_soglia_potenza:.0f} kW</div>', unsafe_allow_html=True)
        with bpa2:
            st.markdown(f'<div class="metric-card"><b>Episodi medi per giornata critica</b><br>'
                        ...
                        f'Auto veloce entra in zona potenza insufficiente</div>', unsafe_allow_html=True)

    with tab_b:
        st.caption(
            f"Condizione: **(rete+batt)/n_attivi < kW_max×coeff** "
            f"AND **(rete+batt)/n_attivi < {potenza_max_stallo:.0f} kW** (P_stallo) "
            f"— nessun filtro velocità auto — 1 episodio per sessione"
        )
        ...
        with bpb1:
            st.markdown(f'<div class="metric-card {b_cls}"><b>Giornate critiche Tipo B</b><br>'
                        ...
                        f'(rete+batt)/stalli_attivi &lt; kW_max×coeff E &lt; {potenza_max_stallo:.0f} kW</div>', unsafe_allow_html=True)
        with bpb2:
            st.markdown(f'<div class="metric-card"><b>Episodi medi per giornata critica</b><br>'
                        f'<span style="font-size:1.6rem;font-weight:700">{res["b_slot_medio"]:.1f} episodi</span><br>'
                        f'Qualsiasi auto penalizzata dalla potenza disponibile</div>', unsafe_allow_html=True)
    ...
    with st.expander("📉 Debug: deficit potenza negli episodi di congestione", expanded=False):
        ...
        def _mostra_delta(dr, dk, label_extra=""):
            ...
            st.markdown("**Δ₁ = kw_max_auto × coeff_picco − kw_per_stallo_disp** (deficit rispetto alla richiesta di picco)")
            cols1 = st.columns(4)
            for col, lbl, val in zip(cols1, ["Media","Mediana","Minimo","Massimo"],
                                     [dr["media"], dr["mediana"], dr["min"], dr["max"]]):
                with col:
                    st.metric(lbl, _fmt(val))
            st.markdown("**Δ₂ = kw_max_auto − kw_per_stallo_disp** (deficit rispetto alla velocità nominale auto)")
            cols2 = st.columns(4)
            ...
            st.caption("Δ₁ include il coefficiente di picco → misura quanto manca rispetto alla richiesta di picco. "
                       "Δ₂ usa kw_max nudo → misura il deficit rispetto alla velocità nominale. Δ₁ ≥ Δ₂ sempre.")
        ...
        if se3["n"] == 0:
            ...
            se_tab3, se_tab4 = st.tabs([
                "Opzione 3 — con coeff (kW_max × coeff)",
                "Opzione 4 — nudo (kW_max)"
            ])
            with se_tab3:
                sc3 = st.columns(4)
            ...
            with se_tab4:
                ...
                for col, lbl, val in zip(sc4, ["Media","Mediana","Minimo","Massimo"],
                    ...
                    with col:
                        ...
                        st.caption(f"≈ {val*5:.1f} min" if val is not None else "")
            st.caption(
                "Opzione 3: divide per kW_max×coeff — ritardo rispetto alla richiesta di picco. "
    ...
    with st.expander("⚡ Debug: vincolo Power Unit", expanded=False):
        ...
        if pu_bind_pct > 20:
            ...
            st.info(f"ℹ️ La Power Unit raramente è il vincolo stringente ({pu_bind_pct:.0f}%). "
                    f"Il collo di bottiglia principale è la potenza rete+batteria.")
